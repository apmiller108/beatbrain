...
SELECT DISTINCT
    ...
    c.name "crate name",
    CASE
      WHEN l.color = '34952' THEN 'low'
      WHEN l.color = '35071' THEN 'medium'
      WHEN l.color = '255' THEN 'High'
      ELSE ''
    END AS "Energy Level",
    tl."location"
FROM
    ...
    where LOWER(c.name) in ('new')
    and l.color IS NOT NULL
    and l.mixxx_deleted = 0
    and l.year > 2022
    and l.bpm > 130
    and l.artist not in ('Elias Garcia')
    order by RANDOM()
```
